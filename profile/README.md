...
아래 여섯 웹 프로젝트는 `vercel.json` 기반 Vite 정적 배포 설정을 갖는다. build command는 `pnpm build`, output directory는 `dist`다.
...
현재 Codex 실행 환경에서는 Vercel CLI/커넥터 인증이 확인되지 않아 production URL 발급은 인증 후 진행한다.
